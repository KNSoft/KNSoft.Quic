...
$DllProject = Join-Path $RepoRoot "KNSoft.Quic\KNSoft.Quic.vcxproj"
$LtcgProject = Join-Path $RepoRoot "KNSoft.Quic.Static.LTCG\KNSoft.Quic.Static.LTCG.vcxproj"
...
function Get-UpstreamPgd([string]$Arch) {
    if ($Arch -eq "x86") {
        return Join-Path $MsQuicRoot "src\bin\winuser\pgo_x86\msquic.pgd"
    }
    if ($Arch -eq "x64") {
        return Join-Path $MsQuicRoot "src\bin\winuser\pgo_x64\msquic.schannel.pgd"
    }
    throw "Upstream PGO data is unavailable for $Arch."
}
...
function Invoke-LtcgBuild(
    ...
    & $VisualStudio.MSBuild @Arguments
    if ($LASTEXITCODE -ne 0) {
        throw "KNSoft.Quic LTCG static library build failed for $Arch."
    }
}
...
function Invoke-MSBuild(
    ...
    $Arguments = @(
        ...
        "/p:Platform=$(Get-Platform $Arch)",
        "/p:SolutionDir=$RepoRoot\",
        "/p:WholeProgramOptimization=$Optimization",
        "/v:minimal",
        "/nologo"
    )
    if ($PgdSource) {
        $Arguments += "/p:KNSoftQuicPGDSource=$PgdSource"
    }

    $Output = @(& $VisualStudio.MSBuild @Arguments 2>&1)
    $Output | ForEach-Object { Write-Host $_ }
    if ($LASTEXITCODE -ne 0) {
        throw "KNSoft.Quic $Optimization build failed for $Arch/$Crt."
    }
    return ,$Output
}
...
function Test-ProfileBuild(
    ...
    $BuildOutput = Invoke-MSBuild $VisualStudio $Arch $Crt "PGOptimize" $Pgd
...
function Test-Performance(
    ...
    $null = Invoke-MSBuild $VisualStudio $Arch $Crt "true"
    $NoneDll = Join-Path $BinaryRoot "KNSoft-None\msquic.dll"
    Copy-CurrentDll $Arch $Crt $NoneDll
    $Variants["KNSoft-None"] = $NoneDll

    if ($Arch -in @("x86", "x64")) {
        $null = Invoke-MSBuild $VisualStudio $Arch $Crt "PGOptimize" (Get-UpstreamPgd $Arch)
...
foreach ($Arch in $Architecture) {
    ...
    Invoke-LtcgBuild $VisualStudio $Arch
    $Tools = @{}
    ...
    foreach ($Crt in $Runtime) {
        ...
        Remove-Item -LiteralPath $InitialPgd, "$InitialPgd.state" -Force -ErrorAction SilentlyContinue
        $null = Invoke-MSBuild $VisualStudio $Arch $Crt "PGInstrument"
